...
namespace PokeCLI
{
    ...
    internal static class Output
    {
        ...
        public static StringBuilder OutputTokens(this CommonTokenStream stream)
        {
            ...
            {
                sb.AppendLine("Token " + token.TokenIndex + " " + token.Type + " " + "channel " +
                              ((Lexer) stream.TokenSource).ChannelNames[token.Channel] + " " +
                              Output.PerformEscapes(token.Text));
            }
        ...
        private static void ParenthesizedAST(this IParseTree tree, StringBuilder sb, CommonTokenStream stream,
            int level = 0)
        {
                ...
                if (inter != null)
                {
                    foreach (IToken t in inter)
                    {
                        ...
                        sb.AppendLine("( " + ((Lexer) stream.TokenSource).ChannelNames[t.Channel] + " text=" +
                                      t.Text.PerformEscapes());
                    }
                }

                StartLine(sb, tree, stream, level);
                sb.AppendLine("( " + ((Lexer) stream.TokenSource).ChannelNames[tok.Symbol.Channel] + " i=" +
                              tree.SourceInterval.a
